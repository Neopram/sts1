...
from app.dependencies import (get_current_user, log_activity,
                              require_room_access)
from app.models import Approval, ApprovalWorkflow, Document, DocumentType, Party, Room, User
...
@router.get("/approvals/workflow/{workflow_id}")
async def get_approval_workflow(
    ...
    """
    Get detailed information about an approval workflow.
    """
    try:
        user_email = current_user.email
        ...
        # Verify user has access to room
        await require_room_access(str(workflow.room_id), user_email, session)

        # Get workflow info using ApprovalService
        approval_service = ApprovalService(session)
        workflow_info = await approval_service.get_workflow_info(workflow_id)
...
@router.post("/approvals/workflow/{workflow_id}/step/{step_id}/approve")
async def approve_workflow_step(
    workflow_id: str,
    step_id: int,
    approve_data: WorkflowStepApproveRequest = WorkflowStepApproveRequest(),
    current_user: User = Depends(get_current_user),
    session: AsyncSession = Depends(get_async_session),
):
    """
    Approve a specific step in an approval workflow.
    """
    try:
        user_email = current_user.email
        user_role = current_user.role
        ...
        # Approve step using ApprovalService
        approval_service = ApprovalService(session)
        ...
        logger.error(
            f"Error approving step {step_id} for workflow {workflow_id}: {e}", exc_info=True
        )
        raise HTTPException(status_code=500, detail="Internal server error")


@router.post("/approvals/workflow/{workflow_id}/step/{step_id}/reject")
async def reject_workflow_step(
    workflow_id: str,
    step_id: int,
    reject_data: WorkflowStepRejectRequest,
    current_user: User = Depends(get_current_user),
    session: AsyncSession = Depends(get_async_session),
):
    """
    Reject a specific step in an approval workflow.
    """
    try:
        user_email = current_user.email
        user_role = current_user.role
        ...
        # Reject step using ApprovalService
        approval_service = ApprovalService(session)
        ...
        logger.error(
            f"Error rejecting step {step_id} for workflow {workflow_id}: {e}", exc_info=True
        )
        raise HTTPException(status_code=500, detail="Internal server error")


@router.get("/approvals/workflows/room/{room_id}")
async def get_room_workflows(
    room_id: str,
    ...
    """
    Get all approval workflows for a room.
    """
    try:
        user_email = current_user.email
        ...
        # Get workflows using ApprovalService
        approval_service = ApprovalService(session)
        workflows = await approval_service.get_workflows_by_room(room_id, status)
        ...
        logger.error(f"Error getting workflows for room {room_id}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Internal server error")
...
class Message(Base):
    ...
    is_public = Column(Boolean, default=True)  # Public/Private message visibility - PHASE 4
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
# Import from sts_operations.py to avoid duplicate class definitions
...
# Remove duplicate class definitions - they are now imported above
# Original duplicate classes were removed to fix SQLAlchemy mapper conflict

# Keep this comment block for reference:
# The models StsOperationSession, OperationParticipant, OperationVessel, and StsOperationCode
# are defined in app/models/sts_operations.py and imported here to avoid duplicate class definitions
# that caused SQLAlchemy mapper conflicts.
